...
const SignUp = ({ display, setDisplay }) => {
  ...
  return (
    <div className='signupPage z-[2] fixed w-full h-screen top-0 left-0 bg-zinc-800/[.4] flex items-center justify-center'>
          ...
          <form onSubmit={handleSubmit(FormSubmit)}>
            <div className="w-full border border-zinc-500 rounded-md">
              ...
              <div className="w-full p-4 text-md relative border-b border-zinc-500 flex items-center justify-center gap-3">
                ...
                <input type="text" className="w-full h-full focus:outline-none text-xl" {...register("username",{required: "Username is required"})}/>
                {errors.username && (<p className='absolute w-full bottom-0 left-[3%] text-red-800 text-sm'>{" "} <i className="ri-information-fill text-[red]"></i>{" "} {errors.username.message}</p>)}
              </div>
              <div className="w-full p-4 text-md relative border-b border-zinc-500 flex items-center justify-center gap-3">
                <label>Phone:- </label>
                <input type="number" className="w-full h-full focus:outline-none text-xl" {...register("mobileNo", {required: "Phone Number is required"})}/>
                {errors.mobileNo && (<p className='absolute w-full bottom-0 left-[3%] text-red-800 text-sm'>{" "} <i className="ri-information-fill text-[red]"></i>{" "} {errors.mobileNo.message}</p>)}
              </div>
              <div className="w-full p-4 text-md relative border-b border-zinc-500 flex items-center justify-center gap-3">
            ...
            <button type="submit" className="w-full text-center bg-[#b17f44] mt-4 text-white rounded-md py-3 cursor-pointer active:scale-95 hover:bg-[rgb(233,140,33)]">Continue</button>
          </form>
        </div>
      </div>
    </div>
